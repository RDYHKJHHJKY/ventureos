...
async function run() {
  ...
  const evidenceReq = {
    ...
    headers: { cookie: `ventureos_session=${session.token}`, 'content-type': 'application/json' },
    body: JSON.stringify({ softwareId: softwarePayload.software.id, type: 'sbom', title: 'CycloneDX SBOM', summary: 'Generated from release pipeline', uri: 'https://example.com/sbom.json', strength: 0.9, freshnessDays: 7, verified: true, workspaceId: 'workspace-1' }),
  };
  ...
  const passportReq = {
    ...
    headers: { cookie: `ventureos_session=${session.token}`, 'content-type': 'application/json' },
    body: JSON.stringify({ softwareId: softwarePayload.software.id, visibility: 'restricted', issuedBy: user.name, accessToken: `${vendorPayload.vendor.id}:workspace-1:restricted`, vendorId: vendorPayload.vendor.id, workspaceId: 'workspace-1' }),
  };
