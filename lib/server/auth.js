...
async function getBcrypt() {
  ...
  try {
    ...
    console.warn('bcrypt import failed, falling back to legacy crypto:', error?.message || error);
    bcryptModule = null;
  }
  return bcryptModule;
}
...
export function canUsePostgresForId(value) {
  return Boolean(value && looksLikeUuid(String(value)));
}

export async function hashPassword(password) {
  // Prefer bcrypt, but fall back to a pbkdf2-based legacy hash if bcrypt is unavailable.
  const bcrypt = await getBcrypt();
  if (bcrypt && typeof bcrypt.hash === "function") {
    try {
      return await bcrypt.hash(password, SALT_ROUNDS);
    } catch (e) {
      console.warn("bcrypt.hash failed, falling back to pbkdf2:", e?.message || e);
    }
  }
  ...
  return Object.fromEntries(
    header
      ...
      .flatMap((part) => {
        ...
        if (!name || /^(?:path|domain|expires|max-age|secure|httponly|samesite|priority|partitioned)$/i.test(name)) {
          return [];
        }
        return [[name, value]];
      })
...
export function sessionCookie(token) {
  return `${SESSION_COOKIE}=${encodeURIComponent(token)}; Path=/; HttpOnly${secureCookieAttributes()}; Max-Age=${60 * 60 * 24 * 14}`;
}

export function csrfCookie(token) {
  return `${CSRF_COOKIE}=${encodeURIComponent(token)}; Path=/; SameSite=Lax${process.env.NODE_ENV === "production" ? "; Secure" : ""}; Max-Age=${60 * 60 * 24 * 14}`;
}

export function clearSessionCookie() {
  return `${SESSION_COOKIE}=; Path=/; HttpOnly${secureCookieAttributes()}; Max-Age=0`;
}

export function clearCsrfCookie() {
  return `${CSRF_COOKIE}=; Path=/; SameSite=Lax${process.env.NODE_ENV === "production" ? "; Secure" : ""}; Max-Age=0`;
}
...
export async function createSession(userId, options = {}) {
  ...
  if (process.env.DATABASE_URL && !workspaceId && canUsePostgresForId(userId)) {
    try {
  ...
  if (process.env.DATABASE_URL && workspaceId && canUsePostgresForId(userId) && canUsePostgresForId(workspaceId)) {
    try {
      await query(
  ...
  await mutateDb((db) => {
    db.sessions.push({ id: createId("session", userId), userId, tokenHash, csrfToken, expiresAt, createdAt: new Date(now).toISOString(), demoMode, workspaceId });
  });
...
async function readUserFromPostgres(userId) {
  if (!canUsePostgresForId(userId)) return null;
...
async function readWorkspaceFromPostgres(workspaceId) {
  if (!canUsePostgresForId(workspaceId)) return null;
...
async function readWorkspaceMembershipsFromPostgres(userId) {
  if (!canUsePostgresForId(userId)) return [];
...
export async function listWorkspacesForUser(userId) {
  if (process.env.DATABASE_URL && canUsePostgresForId(userId)) {
    const result = await query(
...
export async function listWorkspaceUsers(workspaceId) {
  if (process.env.DATABASE_URL && canUsePostgresForId(workspaceId)) {
    const result = await query(
...
export async function findWorkspaceMember(workspaceId, userId) {
  if (process.env.DATABASE_URL && canUsePostgresForId(workspaceId) && canUsePostgresForId(userId)) {
    const result = await query(
...
export async function getSessionContext(req, requestedWorkspaceId) {
  ...
  if (requestedWorkspaceId !== undefined && requestedWorkspaceId !== null) {
    ...
    workspace = db.workspaces.find((item) => item.id === session.workspaceId) || null;
    if (workspace) {
      currentMembership = memberships.find((item) => item.workspaceId === workspace.id) || null;
    }
  }
  ...
  try {
    ...
    const result = await transaction(async (client) => {
      ...
      const wsResult = await client.query(
        `INSERT INTO workspaces (id, owner_id, name, plan)
         VALUES ($1, $2, $3, $4)
         RETURNING id, name, plan`,
        [randomUUID(), user.id, workspaceName || `${name}'s Workspace`, 'starter']
      );
