...
export async function createSession(userId, options = {}) {
    ...
    try {
      await query(
        `INSERT INTO sessions (id, user_id, workspace_id, data, expires_at)
         VALUES ($1, $2, $3, $4, $5)`,
        [tokenHash, userId, workspaceId, JSON.stringify({ csrfToken }), expiresAt]
      );
    } catch (pgError) {
      console.warn("PostgreSQL session creation failed, falling back to file storage:", pgError.message);
      await mutateDb((db) => {
        db.sessions.push({ id: createId("session", userId), userId, tokenHash, csrfToken, expiresAt, createdAt: new Date(now).toISOString() });
      });
  ...
  await mutateDb((db) => {
    db.sessions.push({ id: createId("session", userId), userId, tokenHash, csrfToken, expiresAt, createdAt: new Date(now).toISOString(), workspaceId });
  });
...
export async function validateCsrf(req, ctx) {
  ...
  // Initial auth requests do not have a CSRF cookie yet.
