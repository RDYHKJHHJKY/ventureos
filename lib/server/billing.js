...
export function getWorkspaceUsageSummary(db, workspaceId) {
  ...
  const projectEventCount = (db.projectEvents || []).filter((item) => item.workspaceId === workspaceId).length;

  try {
    const graph = buildTrustGraph(db, workspaceId);
    nodes = graph.nodes.length;
    edges = graph.edges.length;
    const graphTimelineEvents = Array.isArray(graph.timelineEvents) ? graph.timelineEvents : [];
    timelineEvents = projectEventCount > 0 ? Math.max(graphTimelineEvents.length, projectEventCount) : 0;
    riskEvents = workspaceScans.length;
  } catch {
    timelineEvents = projectEventCount;
    riskEvents = workspaceScans.length;
  }
...
function mapWorkspaceMode(billingStatus) {
  const normalized = String(billingStatus || "active").trim().toLowerCase();
  if (normalized === "trialing") return "trial";
  if (normalized === "canceled") return "cancelled";
  if (normalized === "past_due" || normalized === "pending" || normalized === "inactive" || normalized === "incomplete" || normalized === "suspended") return "suspended";
  return "active";
}
...
export function getWorkspaceOverview({ db, mspId, workspaces = listWorkspacesForMsp(db, mspId) }) {
  return workspaces.map((workspace) => {
    ...
    const workspaceMode = mapWorkspaceMode(getMspBillingState(msp).billingStatus);
