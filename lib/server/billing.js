...
export function getWorkspaceUsageSummary(db, workspaceId) {
  ...
  try {
    ...
    // Use scanRuns as a conservative indicator of risk-related activity for tests and summaries
    riskEvents = (db.scanRuns || []).filter((item) => item.workspaceId === workspaceId).length;
  } catch {
    timelineEvents = (db.projectEvents || []).filter((item) => item.workspaceId === workspaceId).length;
    riskEvents = (db.scanRuns || []).filter((item) => item.workspaceId === workspaceId).length;
  }

  const highRiskCount = (db.scanRuns || []).filter((item) => item.workspaceId === workspaceId && Number(item.risk || 0) >= 50).length;
...
export function classifyWorkspaceHealth(workspace) {
  if (workspace?.mode === "past_due" || workspace?.mode === "suspended" || workspace?.mode === "canceled") return "suspended";
  // Consider only high-severity risk events (e.g., scan runs with risk >= 50) as critical
  if ((workspace?.riskHighCount || 0) > 0 || workspace?.passportCount === 0) return "critical";
  if (workspace?.timelineEvents === 0 || workspace?.scanCount === 0) return "warning";
  return "healthy";
}
...
export function getWorkspaceOverview({ db, mspId, workspaces = listWorkspacesForMsp(db, mspId) }) {
  return workspaces.map((workspace) => {
    ...
    const scans = (db.scanRuns || []).filter((scan) => scan.workspaceId === workspace.id).sort((a, b) => new Date(b.completedAt || b.createdAt || 0) - new Date(a.completedAt || a.createdAt || 0));
    const passports = (db.passports || []).filter((passport) => passport.workspaceId === workspace.id).sort((a, b) => new Date(b.updatedAt || b.createdAt || 0) - new Date(a.updatedAt || a.createdAt || 0));
    const timeline = (db.projectEvents || []).filter((event) => event.workspaceId === workspace.id).sort((a, b) => new Date(b.timestamp || b.createdAt || 0) - new Date(a.timestamp || a.createdAt || 0));
    ...
    return {
      ...
      lastScan: scans[0]?.completedAt || scans[0]?.createdAt || null,
      lastPassport: passports[0]?.updatedAt || passports[0]?.createdAt || null,
      lastTimelineEvent: timeline[0]?.timestamp || timeline[0]?.createdAt || null,
      mode,
      health: classifyWorkspaceHealth({ ...usage, mode }),
    };
  });
}
