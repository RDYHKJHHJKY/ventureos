...
export function getWorkspaceUsageSummary(db, workspaceId) {
  ...
  try {
    ...
    const extraGraphEvents = (graph.timelineEvents || []).filter((event) => event?.type === "GRAPH_RECOMPUTED" || event?.type === "GRAPH_TIMELINE_SUMMARY").length;
    timelineEvents = Math.max(0, (graph.timelineEvents || []).length - extraGraphEvents);
