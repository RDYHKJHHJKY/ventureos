...
async function getDataFile() {
  ...
  if (process.env.VENTUREOS_DATA_FILE) {
    ...
    console.log('[data-store] getDataFile using VENTUREOS_DATA_FILE', DATA_FILE);
    return DATA_FILE;
  }
  if (process.env.VERCEL === "1" || process.env.NOW_REGION) {
    DATA_FILE = path.join(os.tmpdir(), "ventureos-db.json");
    console.log('[data-store] getDataFile using tmpdir', DATA_FILE);
    return DATA_FILE;
  }
  ...
  try {
    ...
    console.log('[data-store] getDataFile default', DATA_FILE, 'cwd', process.cwd());
    return DATA_FILE;
  } catch {
    DATA_FILE = path.join(os.tmpdir(), "ventureos-db.json");
    console.log('[data-store] getDataFile fallback tmpdir', DATA_FILE, 'cwd', process.cwd());
    return DATA_FILE;
  }
}
...
async function saveStore(db) {
  const dataFile = await getDataFile();
  console.log('[data-store] saveStore writing file', dataFile, 'sprSoftware ids', (db.sprSoftware || []).map((item) => item.id));
  await mkdir(path.dirname(dataFile), { recursive: true });
...
export async function mutateDb(mutator) {
  ...
  console.log('[data-store] mutateDb before modification sprSoftware ids', (db.sprSoftware || []).map((item) => item.id));
  const result = await mutator(db);
  console.log('[data-store] mutateDb after modification sprSoftware ids', (db.sprSoftware || []).map((item) => item.id));
  await saveStore(db);
  return result;
}

export async function syncRecordToFileStore(collectionName, record) {
  ...
  return mutateDb((db) => {
    ...
    const entries = db[collectionName];
    const existingIndex = entries.findIndex((item) => item.id === record.id);
    if (existingIndex >= 0) {
