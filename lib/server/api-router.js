...
export async function handleApiRequest(req, res) {
  try {
          ...
          async () => {
            ...
            try {
              ...
              if (visibility === "restricted") {
                try {
                  if (process.env.DATABASE_URL) {
                    await verifyRestrictedToken(db, accessToken, { workspaceId: requestedWorkspaceId, evidenceType: normalizedPayload.type });
                  } else {
            ...
            if (process.env.DATABASE_URL) {
              try {
                return await createSprEvidencePostgres({ softwareId, payload, normalizedPayload, type, title, visibility, accessToken, requestedWorkspaceId, software });
              } catch (error) {
    ...
    const evidenceByIdMatch = pathname.match(/^\/api\/spr\/evidence\/([^/]+)$/);
    if (req.method === "GET" && evidenceByIdMatch) {
      requireAuth(ctx);
      const evidenceId = decodeURIComponent(evidenceByIdMatch[1]);
      if (process.env.DATABASE_URL) {
        try {
          const evidence = await getSprEvidenceByIdPostgres(evidenceId);
          if (!evidence) return sendError(res, 404, "NOT_FOUND", "Evidence not found.");
          return sendJson(res, 200, { ok: true, evidence });
        } catch (error) {
          console.warn("SPR evidence Postgres read failed, falling back to file storage:", error?.message || error);
        }
      }
      const db = await readDb();
...
async function getSprEvidenceByIdPostgres(evidenceId, client = null) {
  const result = await runSprQuery(client, `SELECT id, software_id as "softwareId", type, title, summary, source, uri, strength, freshness_days as "freshnessDays", verified, visibility, access_token as "accessToken", workspace_id as "workspaceId", vendor_id as "vendorId", trust_score as "trustScore", numeric_signals as "numericSignals", payload, mime_type as "mimeType", verification_method as "verificationMethod", verification_status as "verificationStatus", verification_details as "verificationDetails", bundle, zk_proof as "zkProof", created_at as "createdAt", updated_at as "updatedAt"
     FROM spr_evidence
     WHERE id = $1
     LIMIT 1`, [evidenceId]);
  return result.rows[0] || null;
