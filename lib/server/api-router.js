...
function createAuditHash(previousHash, type, targetId, payload = {}) {
  const normalized = JSON.stringify({ previousHash: previousHash || null, type, targetId: String(targetId || "").trim() || null, payload });
  let hash = 0;
...
function appendSprAuditLog(db, type, targetId, payload = {}) {
  db.sprAuditLogs = db.sprAuditLogs || [];
  const previousEntry = (db.sprAuditLogs || []).slice(-1)[0] || null;
  const auditHash = createAuditHash(previousEntry?.auditHash || null, type, targetId, payload);
  db.sprAuditLogs.push({
    id: createId("spraudit", `${type}-${targetId || "global"}`),
    type,
    targetId: String(targetId || "").trim() || null,
    payload,
    previousAuditHash: previousEntry?.auditHash || null,
    auditHash,
...
export function verifyAuditChain(entries = []) {
  ...
  for (const entry of entries) {
    const expected = createAuditHash(previousHash, entry.type, entry.targetId, entry.payload || {});
    if (entry.auditHash !== expected) return false;
...
export function verifyPassportEnvelope(passport = {}) {
  const envelope = {
    ...
    evidenceIds: Array.isArray(passport.evidenceIds) ? passport.evidenceIds : [],
    visibility: passport.visibility || null,
    scoringProfile: passport.scoringProfile || null,
    issuedAt: passport.issuedAt || null,
  };
  const expected = createAuditHash(null, "passport.envelope", passport.id, envelope);
  return passport.passportEnvelopeHash === expected;
}
...
function normalizeSprEvidence(evidence) {
  ...
  return {
    type: normalizedType,
    title: String(evidence.title || evidence.summary || "Evidence").trim(),
    summary: String(evidence.summary || "").trim(),
    source: String(evidence.source || "manual").trim(),
    uri: String(evidence.uri || "").trim(),
    strength: Number(evidence.strength || 0),
    freshnessDays: Number(evidence.freshnessDays || 0),
    verified: Boolean(evidence.verified === true),
    visibility: String(evidence.visibility || "public").trim().toLowerCase() || "public",
    accessToken: String(evidence.accessToken || "").trim() || null,
    verificationMethod: String(evidence.verificationMethod || "").trim() || null,
    verificationStatus: String(evidence.verificationStatus || (Boolean(evidence.verified === true) ? "verified" : "pending")).trim().toLowerCase() || "pending",
...
export async function handleApiRequest(req, res) {
  try {
    ...
    if (req.method === "POST" && pathname === "/api/spr/software") {
      ...
      if (!vendorId) return sendError(res, 400, "VALIDATION_ERROR", "Vendor is required.");
      const software = await mutateDb((db) => {
        const record = {
        ...
        db.sprSoftware.push(record);
        return record;
      });
      return sendJson(res, 201, { ok: true, software });
    }
    ...
    if (req.method === "POST" && pathname === "/api/spr/evidence") {
      ...
      const parsedToken = parseScopedAccessToken(accessToken, String(payload.vendorId || software.vendorId || "").trim() || null, String(payload.workspaceId || software.workspaceId || "").trim() || null);
      ...
      if (visibility === "restricted" && parsedToken.workspaceId && String(payload.workspaceId || software.workspaceId || "").trim() && parsedToken.workspaceId !== String(payload.workspaceId || software.workspaceId || "").trim()) {
      ...
      const evidence = await mutateDb((db2) => {
        ...
        appendSprAuditLog(db2, "evidence.created", record.id, { visibility, freshnessDays, softwareId });
        return record;
      });
    ...
    if (req.method === "POST" && softwareSignalsMatch) {
      ...
      const software = db.sprSoftware.find((item) => item.id === softwareId);
      if (!software) return sendError(res, 404, "NOT_FOUND", "Software not found.");
      const signalType = String(payload.type || "monitoring").trim().toLowerCase();
      const severity = String(payload.severity || "info").trim().toLowerCase();
      const source = String(payload.source || "manual").trim();
      if ((signalType === "cve" || signalType === "breach") && !payload.summary) return sendError(res, 400, "VALIDATION_ERROR", "Signal summary is required.");
      if (signalType === "breach" && !source) return sendError(res, 400, "VALIDATION_ERROR", "Signal source is required.");
      const signal = await mutateDb((db2) => {
        const record = {
          id: createId("sprsignal", `${softwareId}-${signalType}`),
          softwareId,
          type: signalType,
          severity,
        ...
        appendSprAuditLog(db2, "signal.created", record.id, { type: signalType, severity, source });
        return record;
      });
    ...
    if (req.method === "POST" && evidenceBundleMatch) {
      ...
      const updated = await mutateDb((db) => {
        ...
        if (!record) return null;
        record.bundle = {
          encrypted: Boolean(payload.encrypted === true),
          recipients: sanitizeList(payload.recipients),
          selectiveDisclosure: Boolean(payload.selectiveDisclosure === true),
        };
        record.updatedAt = new Date().toISOString();
        return record;
      });
    ...
    if (req.method === "POST" && pathname === "/api/spr/passports/issue") {
      ...
      const vendor = db.sprVendors.find((item) => item.id === software.vendorId) || null;
      const evidence = db.sprEvidence.filter((item) => item.softwareId === softwareId);
      const { trustScore, confidenceScore, verdict, riskCategory } = computeSprPassportScore(software, evidence, vendor);
      const visibility = String(payload.visibility || "public").trim().toLowerCase();
      const allowedVisibility = ["public", "private", "restricted"].includes(visibility) ? visibility : "public";
      const accessToken = allowedVisibility === "restricted" ? String(payload.accessToken || "").trim() || null : null;
      const workspaceId = String(payload.workspaceId || "").trim() || null;
      const vendorId = String(payload.vendorId || software.vendorId || "").trim() || null;
      const parsedToken = parseScopedAccessToken(accessToken, vendorId, workspaceId);
      if (allowedVisibility === "restricted" && !accessToken) {
        return sendError(res, 400, "VALIDATION_ERROR", "Restricted passports require an access token.");
      }
      if (allowedVisibility === "restricted" && parsedToken.vendorId && vendorId && parsedToken.vendorId !== vendorId) {
        return sendError(res, 403, "FORBIDDEN", "Restricted passport token scope is invalid for this vendor.");
      }
      if (allowedVisibility === "restricted" && parsedToken.workspaceId && workspaceId && parsedToken.workspaceId !== workspaceId) {
        return sendError(res, 403, "FORBIDDEN", "Restricted passport token scope is invalid for this workspace.");
      }
      const freshnessViolations = evidence.filter((item) => Number(item.freshnessDays || 0) > 180);
      if (allowedVisibility === "restricted" && freshnessViolations.length > 0) {
        return sendError(res, 400, "VALIDATION_ERROR", "Restricted passports require fresh evidence.");
      }
      const now = new Date();
      const passport = await mutateDb((db2) => {
        const record = {
          ...
          accessToken,
          workspaceId,
          scoringProfile: String(payload.profile || "default").trim().toLowerCase() || "default",
          ...
          verdict,
          riskCategory,
          status: allowedVisibility === "public" ? "Active" : allowedVisibility === "restricted" ? "Restricted" : "Draft",
          issuedAt: now.toISOString().slice(0, 10),
          expiresAt: new Date(now.getTime() + 90 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
          evidenceSummary: evidence.length > 0 ? `${evidence.length} evidence item(s) reviewed.` : "No evidence submitted yet.",
          issuedBy: String(payload.issuedBy || ctx.user?.name || "system").trim(),
          createdAt: now.toISOString(),
          updatedAt: now.toISOString(),
        };
        db2.sprPassports.push(record);
        record.passportEnvelopeHash = createAuditHash(null, "passport.envelope", record.id, {
          identity: record.softwareId,
          evidenceIds: record.evidenceIds,
          visibility: record.visibility,
          scoringProfile: record.scoringProfile,
          issuedAt: record.issuedAt,
        });
        appendSprAuditLog(db2, allowedVisibility === "restricted" ? "passport.access_granted" : "passport.issued", record.id, { visibility: allowedVisibility, accessToken, scoringProfile: record.scoringProfile, workspaceId });
        return record;
      });
    ...
    if (req.method === "GET" && passportReplayMatch) {
      ...
      return sendJson(res, 200, { ok: true, passportId, events, chainValid: verifyAuditChain(events) });
    }
